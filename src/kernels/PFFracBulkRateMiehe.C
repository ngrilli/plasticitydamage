...
template <>
InputParameters
validParams<PFFracBulkRateMiehe>()
{
...
PFFracBulkRateMiehe::PFFracBulkRateMiehe(const InputParameters & parameters)
  : KernelValue(parameters),
    _gc_prop(getMaterialProperty<Real>("gc_prop_var")),
    _G0_pos(getMaterialProperty<Real>("G0_var")),
    _G0_pos_old(getMaterialPropertyOld<Real>("G0_var")),
    _dG0_pos_dstrain(isParamValid("dG0_dstrain_var")
                         ? &getMaterialProperty<RankTwoTensor>("dG0_dstrain_var")
                         : NULL),
    _dG0_pos_dstrain_old(isParamValid("dG0_dstrain_var")
                         ? &getMaterialPropertyOld<RankTwoTensor>("dG0_dstrain_var")
                         : NULL),
    _damage(getMaterialProperty<Real>("damage")),
    _damage_old(getMaterialPropertyOld<Real>("damage")),
    _x_bracket(getMaterialProperty<Real>("x_bracket")),
    _x_bracket_old(getMaterialPropertyOld<Real>("x_bracket")),
...
Real
PFFracBulkRateMiehe::precomputeQpResidual()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real c_old = _damage_old[_qp];

  return -_damage_old[_qp];
}

Real
PFFracBulkRateMiehe::precomputeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
...
Real
PFFracBulkRateMiehe::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
