...
PFFracBulkRateNob::PFFracBulkRateNob(const InputParameters & parameters)
...
Real
PFFracBulkRateNob::precomputeQpResidual()
{
...
Real
PFFracBulkRateNob::precomputeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real beta = _d2c_dx2[_qp]+_d2c_dy2[_qp]+_d2c_dz2[_qp];
...
Real
PFFracBulkRateNob::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
