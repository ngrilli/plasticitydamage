...
template <>
InputParameters
validParams<PFFracBulkRateCAux>()
{
  ...
  params.addRequiredCoupledVar("c_copy","Copy of c as auxvariable");

  return params;
}

PFFracBulkRateCAux::PFFracBulkRateCAux(const InputParameters & parameters)
...
Real
PFFracBulkRateCAux::precomputeQpResidual()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _c_copy[_qp];
...
Real
PFFracBulkRateCAux::precomputeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _c_copy[_qp];
  const Real beta = _d2c_dx2[_qp]+_d2c_dy2[_qp]+_d2c_dz2[_qp];
...
Real
PFFracBulkRateCAux::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
