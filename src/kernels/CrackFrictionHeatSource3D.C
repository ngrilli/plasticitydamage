...
template <>
InputParameters
validParams<CrackFrictionHeatSource3D>()
{
...
CrackFrictionHeatSource3D::CrackFrictionHeatSource3D(const InputParameters & parameters)
...
Real
CrackFrictionHeatSource3D::computeQpResidual()
{
  return -_heat_source_rate[_qp] * _test[_i][_qp];
}

Real
CrackFrictionHeatSource3D::computeQpJacobian()
{
  return -0.0 * _phi[_j][_qp] * _test[_i][_qp];
}

Real
CrackFrictionHeatSource3D::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real val;
