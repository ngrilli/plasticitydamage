...
template <>
InputParameters
validParams<TimeDerivativeDamage>()
{
...
TimeDerivativeDamage::TimeDerivativeDamage(const InputParameters & parameters)
...
Real
TimeDerivativeDamage::precomputeQpResidual()
{
  ...
  return _u_dot[_qp]-((std::abs(x) + x) / 2.0) / _visco;
}

Real
TimeDerivativeDamage::precomputeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real beta = _d2c_dx2[_qp]+_d2c_dy2[_qp]+_d2c_dz2[_qp];
  ...
  return (MathUtils::sign(x) + 1.0) / 2.0 * (2.0 * _G0_pos[_qp] / gc + 1.0 / _l) / _visco *
         _phi[_j][_qp] + _phi[_j][_qp] * _du_dot_du[_qp];
}

Real
TimeDerivativeDamage::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
