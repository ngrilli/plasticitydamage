...
template <>
InputParameters
validParams<ArrheniusMassFractionRateLimit2species>()
{
  InputParameters params = validParams<Kernel>();
  params.addClassDescription("Chemical reaction mass fraction decrease rate: Arrhenius equation with rate limit for two species");
...
ArrheniusMassFractionRateLimit2species::ArrheniusMassFractionRateLimit2species(const InputParameters & parameters)
...
Real
ArrheniusMassFractionRateLimit2species::computeQpResidual()
{
  return _test[_i][_qp] * _mass_fraction_rate[_qp];
}

Real
ArrheniusMassFractionRateLimit2species::computeQpJacobian()
{
  return _test[_i][_qp] * _phi[_j][_qp] * _dmass_fraction_rate_dmass_fraction_1[_qp];
}

Real
ArrheniusMassFractionRateLimit2species::computeQpOffDiagJacobian(unsigned int jvar)
{
