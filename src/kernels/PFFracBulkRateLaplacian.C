...
template <>
InputParameters
validParams<PFFracBulkRateLaplacian>()
{
  InputParameters params = validParams<Kernel>(); // Value
...
PFFracBulkRateLaplacian::PFFracBulkRateLaplacian(const InputParameters & parameters)
...
Real
PFFracBulkRateLaplacian::computeQpResidual()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real x = - _l * _grad_u[_qp] * _grad_test[_i][_qp] + 2.0 * (1.0 - c) * _G0_pos[_qp] / gc - c / _l;

  return -((std::abs(x) + x) / 2.0) / _visco;
}

Real
PFFracBulkRateLaplacian::computeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real x = - _l * _grad_u[_qp] * _grad_test[_i][_qp] + 2.0 * (1.0 - c) * _G0_pos[_qp] / gc - c / _l;

  return ( (MathUtils::sign(x) + 1.0) / 2.0 ) * ( _l * _grad_phi[_j][_qp] * _grad_test[_i][_qp] + (2.0 * _G0_pos[_qp] / gc + 1.0 / _l) *_phi[_j][_qp] ) / _visco;
}

Real
PFFracBulkRateLaplacian::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
