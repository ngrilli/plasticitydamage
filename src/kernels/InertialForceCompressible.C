...
InputParameters validParams<InertialForceCompressible>()
{
  ...
  params.addClassDescription("Calculates the residual for the interial force (M*accel) and the contribution of mass dependent Rayleigh damping and HHT time integration scheme [eta*M*((1+alpha)vel-alpha*vel_old)] for compressible solids");
  ...
  params.addRequiredCoupledVar("density","density variable");
...
InertialForceCompressible::InertialForceCompressible(const InputParameters & parameters) :
    Kernel(parameters),
    _density(coupledValue("density")), // not anymore a material property
    _u_old(valueOld()),
...
Real
InertialForceCompressible::computeQpResidual()
{
  if (_dt == 0)
    return 0;
  else
...
Real
InertialForceCompressible::computeQpJacobian()
{
  if (_dt == 0)
    return 0;
  else
