...
PFFracBulkRatePlastic::PFFracBulkRatePlastic(const InputParameters & parameters):
...
Real
PFFracBulkRatePlastic::precomputeQpResidual()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real x = _l * _betaval[_qp] + 2.0 * (1.0 - c) * (_W0e[_qp]+_W0p[_qp]-_Wc) / gc - c / _l;

  return -((std::abs(x) + x) / 2.0) / _visco;
}

Real
PFFracBulkRatePlastic::precomputeQpJacobian()
{
  const Real gc = _gc_prop[_qp];
  const Real c = _u[_qp];
  const Real x = _l * _betaval[_qp] + 2.0 * (1.0 - c) * (_W0e[_qp]+_W0p[_qp]-_Wc) / gc - c / _l;

  return (MathUtils::sign(x) + 1.0) / 2.0 * (2.0 * (_W0e[_qp]+_W0p[_qp]-_Wc) / gc + 1.0 / _l) / _visco *
         _phi[_j][_qp];
}

Real
PFFracBulkRatePlastic::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
  ...
  if (disp_flag && _dW0e_dstrain != NULL && _dW0p_dstrain != NULL)
  {
    ...
    for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
      val += ((*_dW0e_dstrain)[_qp](c_comp, i) + (*_dW0e_dstrain)[_qp](i, c_comp)) / 2.0 *
             _grad_phi[_j][_qp](i);
    for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
      val += ((*_dW0p_dstrain)[_qp](c_comp, i) + (*_dW0p_dstrain)[_qp](i, c_comp)) / 2.0 *
             _grad_phi[_j][_qp](i);
