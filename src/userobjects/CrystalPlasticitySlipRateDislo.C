...
InputParameters validParams<CrystalPlasticitySlipRateDislo>()
{
  ...
  params.addClassDescription("Dislocation based constitutive model slip rate class.  Override the virtual functions in your class");
  return params;
}

CrystalPlasticitySlipRateDislo::CrystalPlasticitySlipRateDislo(const InputParameters & parameters) :
...
void
CrystalPlasticitySlipRateDislo::readFileFlowRateParams()
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    for (unsigned int j = 0; j < _num_slip_sys_flowrate_props; ++j)
      if (!(file >> vec[j]))
        mooseError("Error CrystalPlasticitySlipRateDislo: Premature end of slip_sys_flow_rate_param file");
...
void
CrystalPlasticitySlipRateDislo::getFlowRateParams()
{
  if (_flowprops.size() <= 0)
    mooseError("CrystalPlasticitySlipRateDislo: Error in reading flow rate  properties: Specify input in .i file or a slip_sys_flow_prop_file_name");
  ...
  for (unsigned int i = 0; i < _flowprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs != std::floor(vs) || ve != std::floor(ve))
      mooseError("CrystalPlasticitySlipRateDislo: Error in reading flow props: Values specifying start and end number of slip system groups should be integer");

    is = static_cast<unsigned int>(vs);
    ie = static_cast<unsigned int>(ve);

    if (is > ie)
      mooseError("CrystalPlasticitySlipRateDislo: Start index is = ", is, " should be greater than end index ie = ", ie, " in flow rate parameter read");
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    {
      mooseWarning( "CrystalPlasticitySlipRateDislo: Non-positive flow rate parameters ", _a0(i), ",", _xm(i) );
      break;
    }
  }
}
...
void
CrystalPlasticitySlipRateDislo::calcFlowDirection(unsigned int qp, std::vector<RankTwoTensor> & flow_direction) const
{
...
bool
CrystalPlasticitySlipRateDislo::calcSlipRate(unsigned int qp, Real dt, std::vector<Real> & val) const
{
  DenseVector<Real> tau(_variable_size);
  DenseVector<Real> rho(_variable_size);
  DenseVector<Real> tauth(_variable_size);

  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    rho(i) = _mat_prop_state_var[qp][i];
    tau(i) = _pk2[qp].doubleContraction(_flow_direction[qp][i]);
    tauth(i) = 100.0 * std::sqrt( rho(i) );
  }

  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    // val[i] is the plastic slip rate
    val[i] = _a0(i) * std::pow(std::abs(tau(i) / tauth(i)), 1.0 / _xm(i)) * copysign(1.0, tau(i));
...
bool
CrystalPlasticitySlipRateDislo::calcSlipRateDerivative(unsigned int qp, Real /*dt*/, std::vector<Real> & val) const
{
  DenseVector<Real> tau(_variable_size);
  DenseVector<Real> rho(_variable_size);
  DenseVector<Real> tauth(_variable_size);
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
    val[i] = _a0(i) / _xm(i) * std::pow(std::abs(tau(i) / tauth(i)), 1.0 / _xm(i) - 1.0) / tauth(i);
