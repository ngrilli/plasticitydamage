...
InputParameters validParams<FiniteStrainCrystalPlasticityEOSquadC0>()
{
  ...
  params.addClassDescription("Crystal Plasticity base class: FCC system with power law flow rule implemented: bulk viscosity with quadratic C0 and equation of state");
...
FiniteStrainCrystalPlasticityEOSquadC0::FiniteStrainCrystalPlasticityEOSquadC0(const InputParameters & parameters) :
...
void
FiniteStrainCrystalPlasticityEOSquadC0::calcResidual( RankTwoTensor &resid )
{
  RankTwoTensor iden, ce, ee, ce_pk2, eqv_slip_incr, pk2_new;
  RankTwoTensor velocity_gradient;
  ...
  velocity_gradient = ( _deformation_gradient[_qp] - _deformation_gradient_old[_qp] ) / _dt;

  pk2_new.addIa( - _C0 * velocity_gradient.trace() * velocity_gradient.trace() );
  pk2_new.addIa( - _C1 * velocity_gradient.trace() );
