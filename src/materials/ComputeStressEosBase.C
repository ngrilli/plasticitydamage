...
validParams<ComputeStressEosBase>()
{
  ...
  params.addRequiredParam<Real>("bulk_modulus_ref", "reference bulk modulus in Birch-Murnaghan EOS");
...
ComputeStressEosBase::computeQpProperties()
{
  ...
  EOS_stress.zero();
  // Birch-Murnaghan EOS
  EOS_stress.addIa((_Bulk_Modulus_Ref/_n_Murnaghan) * (1.0 - std::pow(1.0 / _deformation_gradient[_qp].det(),_n_Murnaghan)));

  // Calculate rate of change of the specific volume
  dspecific_volume_dt = (_deformation_gradient[_qp].det() - _deformation_gradient_old[_qp].det()) / _dt;
