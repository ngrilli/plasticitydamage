...
InputParameters validParams<FiniteStrainCrystalPlasticityBulkViscosity>()
{
  ...
  params.addClassDescription("Crystal Plasticity base class: FCC system with power law flow rule implemented: bulk viscosity");
...
FiniteStrainCrystalPlasticityBulkViscosity::FiniteStrainCrystalPlasticityBulkViscosity(const InputParameters & parameters) :
...
void
FiniteStrainCrystalPlasticityBulkViscosity::calcResidual( RankTwoTensor &resid )
{
  ...
  pk2_new.addIa( - _C0 * velocity_gradient.trace() * abs(velocity_gradient.trace()) );
