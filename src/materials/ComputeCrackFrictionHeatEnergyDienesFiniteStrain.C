...
template <>
InputParameters
validParams<ComputeCrackFrictionHeatEnergyDienesFiniteStrain>()
{
...
// dcdx and dcdy should be calculated using the AuxKernel VariableGradientComponent
// and the option "use_displaced_mesh = true" in that AuxKernel
...
ComputeCrackFrictionHeatEnergyDienesFiniteStrain::ComputeCrackFrictionHeatEnergyDienesFiniteStrain(const InputParameters & parameters)
...
void
ComputeCrackFrictionHeatEnergyDienesFiniteStrain::computeQpProperties()
{
...
// Finite strain velocity gradient L: \dot{F} = L F
  _velocity_gradient[_qp] = ( _deformation_gradient[_qp] - _deformation_gradient_old[_qp] ) / _dt;
  _velocity_gradient[_qp] = _velocity_gradient[_qp] * _deformation_gradient_old[_qp].inverse();
