...
InputParameters validParams<FiniteStrainCrystalPlasticityEOSBulkViscosityFdot>()
{
  ...
  params.addClassDescription("Crystal Plasticity base class: FCC system with power law flow rule implemented: bulk viscosity (TrD = Tr(Fdot)) and equation of state");
...
FiniteStrainCrystalPlasticityEOSBulkViscosityFdot::FiniteStrainCrystalPlasticityEOSBulkViscosityFdot(const InputParameters & parameters) :
...
void
FiniteStrainCrystalPlasticityEOSBulkViscosityFdot::calcResidual( RankTwoTensor &resid )
{
  ...
  trD = ( _deformation_gradient[_qp].trace() - _deformation_gradient_old[_qp].trace() ) / _dt;
